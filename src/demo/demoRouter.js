...

  {
    ...
    handler: () => {
      ...
      const snapshotData = {
        exportedAt: now,
        tables: {
          categories: collections.categories.list(),
          products: collections.products.list(),
          suppliers: collections.suppliers.list(),
          customers: collections.customers.list(),
          sales: collections.sales.list(),
          invoices: collections.invoices.list(),
          purchaseOrders: collections.purchaseOrders.list(),
          expenseCategories: collections.expenseCategories.list(),
          expenses: collections.expenses.list(),
          businessSettings: [singletons.businessSettings],
        },
      };

      const record = {
        id: generateId(),
        fileName: `smartbizz-backup-demo-${Date.now()}.json`,
        fileSizeBytes: JSON.stringify(snapshotData).length,
        status: "Completed",
        startedAt: now,
        completedAt: now,
        triggeredByName: DEMO_USER.fullName,
      };

      singletons.backups = [record, ...singletons.backups];
      return created({ ...record, data: snapshotData });
    },
  },
];
